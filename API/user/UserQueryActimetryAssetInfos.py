...
post_schema = api.schema_model(
    ...
    {
        "properties": {
            "asset_uuid": {"type": "string", "location": "json"},
            "access_token": {"type": "string", "location": "json"},
        }
    },
)


class UserQueryActimetryAssetInfos(UserQueryBase):
    ...
    @api.doc(
        description="Query information about stored file",
        responses={
            200: "Success - Return information about file",
            400: "Bad request",
    ...
    @api.doc(
        description="Query information about multiple assets at the same time or update an existing asset "
        ...
        responses={
            200: "Success - Return information about file assets",
    ...
    def post(self):
        ...
        if "assets" in request.json:
            ...
            for asset in assets:
                ...
                if not Globals.service.has_access_to_asset(
                    access_token=asset["access_token"], asset_uuid=asset["asset_uuid"]
                ):
                    return gettext("Access denied for at least one requested asset"), 403
                requested_assets_uuids.append(asset["asset_uuid"])

            # Query assets data
            assets = ActimetryAsset.get_assets_for_uuids(requested_assets_uuids)
            ...
            asset = ActimetryAsset.get_asset_for_uuid(asset_json["asset_uuid"])
