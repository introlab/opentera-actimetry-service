...
post_schema = api.schema_model('worker',
                               {'properties':
                                   {
                                       'key':
                                           {
...
class UserQueryActimetryProcessing(Resource):
    ...
    @api.doc(description='Get the status of a processing algorithm worker task',
             responses={200: 'Success - returns status.',
    ...
    def get(self):
        ...
        if log.worker_owner_type != WorkerOwnerType.OWNER_USER or log.worker_owner_uuid != current_user_client.user_uuid:
            return gettext('Forbidden access to that worker'), 403

        # All good ! Return status
        return log.to_json(), 200

    @api.doc(description='Starts a new processing algorithm worker task',
             responses={200: 'Success - Return information about started task',
    ...
    def post(self):
        ...
        participant = current_user_client.do_get_request_to_backend(path='/api/user/participants',
                                                                    params={'participant_uuid': json_worker['participant_uuid']})
        if not participant:
        ...
        # Ok, start worker now!
        script_name = 'workers/algorithms/' + json_worker['key'] + '.py'  # TODO: Another way to find algorithms scripts?
        database_path = Globals.service.config_man.service_config['database_directory'] + os.sep + database.database_uuid
        (work_uuid, worker_status) = Globals.worker_man.start_processing_worker(script=script_name,
